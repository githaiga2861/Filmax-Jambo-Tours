...
(async function(){
  try {
    ...
    if(!supa)return;
    const {data,error}=await supa.from('packages').select('name,slug,badge,category,tier,duration_days,duration_nights,price_high_season,price_duo,short_highlights,card_bg_image_url,hero_image_url,detail_page_url').eq('is_published',true).order('created_at',{ascending:false});
    if(error||!data||!data.length)return;
    data.forEach(function(pkg){
      const price=pkg.price_high_season||pkg.price_duo||0;
      const days=pkg.duration_days||0, nights=pkg.duration_nights||0;
      const page=pkg.detail_page_url||('/packages/'+pkg.slug+'/');
      const exists=allPackages.some(function(p){return p.page===page;});
      if(exists)return;
      allPackages.push({badge:pkg.badge||null,duration:days?days+' Days · '+nights+' Nights':'',name:pkg.name,price:'$'+Number(price).toLocaleString(),page:page,tier:pkg.tier||'mid',features:(pkg.short_highlights&&pkg.short_highlights.length?pkg.short_highlights:[]),cardBg:pkg.card_bg_image_url||pkg.hero_image_url||null});
    });
  } catch(_){}
})();
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  setInterval(function(){currentSet[0]=(currentSet[0]+3)%allPackages.length;renderCard(card0,currentSet[0],false,true,0);},32000);
  setTimeout(function(){setInterval(function(){currentSet[1]=(currentSet[1]+3)%allPackages.length;renderCard(card1,currentSet[1],true,true,1);},32000);},11000);
  setTimeout(function(){setInterval(function(){currentSet[2]=(currentSet[2]+3)%allPackages.length;renderCard(card2,currentSet[2],false,true,2);},32000);},21000);
});
  ...
  (function populateGalleryFromSupabase(){
    ...
    fetch(SUPA_URL+'/rest/v1/gallery_items?select=*&order=tile_index.asc.nullslast,sort_order.asc', {
      ...
      data.filter(function(row){ return row.tile_index!==null; }).forEach(function(row){
        ...
        el.setAttribute('data-src', row.image_url);
        el.setAttribute('aria-label', 'Open '+row.title);
        var eyebrowEl=el.querySelector('.gc-eyebrow');
        var titleEl=el.querySelector('.gc-title');
        if(eyebrowEl) eyebrowEl.textContent=row.eyebrow;
        if(titleEl) titleEl.textContent=row.title;
        if(row.media_type==='video'){
          var vid=el.querySelector('.gc-video');
          if(vid){ vid.dataset.src=row.image_url; if(vid.src){ vid.src=row.image_url; } }
        } else {
          var img=el.querySelector('.gc-media:not(.gc-video)');
      ...
      if(mobileDup && sourceTile){
        mobileDup.setAttribute('data-type', sourceTile.getAttribute('data-type'));
        mobileDup.setAttribute('data-src', sourceTile.getAttribute('data-src'));
        mobileDup.setAttribute('aria-label', sourceTile.getAttribute('aria-label'));
        var srcImg=sourceTile.querySelector('.gc-media:not(.gc-video)');
        var dupImg=mobileDup.querySelector('.gc-media:not(.gc-video)');
        if(srcImg && dupImg){ dupImg.src=srcImg.src; dupImg.alt=srcImg.alt; }
        var srcEyebrow=sourceTile.querySelector('.gc-eyebrow');
        var dupEyebrow=mobileDup.querySelector('.gc-eyebrow');
        if(srcEyebrow && dupEyebrow) dupEyebrow.textContent=srcEyebrow.textContent;
        var srcTitle=sourceTile.querySelector('.gc-title');
